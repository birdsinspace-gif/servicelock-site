...
function getResendFromEmail() {
  return (
    process.env.SERVICELOCK_RUNTIME_DIAGNOSTIC_FROM_EMAIL ||
    process.env.SERVICELOCK_ONBOARDING_FROM_EMAIL ||
    ""
  );
}
...
function getReplyToEmail() {
  return (
    process.env.SERVICELOCK_RUNTIME_DIAGNOSTIC_REPLY_TO ||
    process.env.SERVICELOCK_ONBOARDING_REPLY_TO ||
    DEFAULT_REPLY_TO
  );
}
...
async function runResendDiagnostic(req) {
  ...
  const toEmail =
    process.env.SERVICELOCK_RUNTIME_DIAGNOSTIC_TO_EMAIL ||
    process.env.TRIAL_ALERT_EMAIL ||
    DEFAULT_REPLY_TO;
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: `ServiceLock Diagnostic <${fromEmail}>`,
      to: [toEmail],
      reply_to: getReplyToEmail(),
      subject: `ServiceLock runtime diagnostic ${new Date().toISOString()}`,
      html: `<p>ServiceLock root Vercel function diagnostic succeeded.</p><p>Path: ${req.url}</p>`,
    }),
  });
...
async function runRedisDiagnostic() {
  ...
  const key = `runtime-diagnostic:servicelock:${Date.now()}`;
